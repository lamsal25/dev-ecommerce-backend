...
def createMarketplaceProduct(request):
    try:
        if(request.user.role == "vendor"):
            return Response({"error": "Only users with 'user' role can create marketplace products"}, status=status.HTTP_403_FORBIDDEN)
        data = request.data

        # Extract related objects (IDs must be passed in request)
        category = Category.objects.get(id=data['category'])
        user = request.user
        image = request.FILES.get("image")
        topImage = request.FILES.get("topImage")
        bottomImage = request.FILES.get("bottomImage")
        leftImage = request.FILES.get("leftImage")
        rightImage = request.FILES.get("rightImage")

        main_image_url = upload_image_to_supabase(image) if image else None
        top_image_url = upload_image_to_supabase(topImage) if topImage else None
        bottom_image_url = upload_image_to_supabase(bottomImage) if bottomImage else None
        left_image_url = upload_image_to_supabase(leftImage) if leftImage else None
        right_image_url = upload_image_to_supabase(rightImage) if rightImage else None
        ...
        serializer = MarketPlaceProductSerializer(product)
        return Response({
        ...
        print("Error in create_marketplace_product:", str(e))
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    
# Update Marketplace Product
@api_view(['PUT'])
def updateMarketplaceProduct(request, pk):
    try:
        product = MarketPlaceProduct.objects.get(pk=pk)
    ...
    try:
        ...
        product.isAvailable = request.data.get("isAvailable", str(product.isAvailable)) == "true"
        product.review = request.data.get("review", product.review)
        product.rating = request.data.get("rating", product.rating)

        # Handle updated images
        def handle_image(field_name, existing_value):
            image_file = request.FILES.get(field_name)
            return upload_image_to_supabase(image_file) if image_file else existing_value

        product.image = handle_image("image", product.image)
        product.topImage = handle_image("topImage", product.topImage)
        product.bottomImage = handle_image("bottomImage", product.bottomImage)
        product.leftImage = handle_image("leftImage", product.leftImage)
        product.rightImage = handle_image("rightImage", product.rightImage)
        ...
        print("Marketplace Product update exception:")
        traceback.print_exc()
        return JsonResponse({"error": str(e)}, status=500)
    
# Get Marketplace Product by Category   
@api_view(['GET'])
def getMarketPlaceProductByCategory(request, category_id):
    try:
       
        products = MarketPlaceProduct.objects.filter(category_id=category_id)
        ...
        print("Marketplace Product fetch by category exception:")
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
@api_view(['GET'])
@permission_classes([AllowAny])
def getMarketPlaceProductbyID(request, pk):
    """
    Fetch a single product by its primary key (ID)
    """
    try:
...
@api_view(['GET'])
def getMarketplaceProductByUser(request):
    user_id = request.user.id
    print(user_id)
    products = MarketPlaceProduct.objects.filter(user_id=user_id)
    serializer = MarketPlaceProductSerializer(products, many=True)
    return Response({
        "message": "Marketplace Products fetched successfully",
        "data": serializer.data
...
@api_view(['DELETE'])
def deleteMarketplaceProduct(request, pk):
    try:
        product = MarketPlaceProduct.objects.get(pk=pk)
        product.delete()
        return Response({"message": "Marketplace Product deleted successfully"}, status=status.HTTP_204_NO_CONTENT)
    except MarketPlaceProduct.DoesNotExist:
        return Response({"error": "Marketplace Product not found"}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        print("Marketplace Product delete exception:")
        traceback.print_exc()
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    
@api_view(['GET'])
@permission_classes([AllowAny])
def getApprovedMarketplaceProducts(request):
    """
    Get all approved marketplace products
    """
    try:
        products = MarketPlaceProduct.objects.filter(isApproved=True)
        serializer = MarketPlaceProductSerializer(products, many=True)
        return Response({
            "message": "Approved marketplace products fetched successfully",
            "data": serializer.data
        }, status=status.HTTP_200_OK)
    except Exception as e:
        print("Error fetching approved marketplace products:", e)
        traceback.print_exc()
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
@api_view(['GET'])
@permission_classes([AllowAny])
def getApprovedMarketplaceProductsByCategory(request,category_id):
    """
    Get all approved marketplace products
    """
    try:
        products = MarketPlaceProduct.objects.filter(isApproved=True,category_id=category_id)
        serializer = MarketPlaceProductSerializer(products, many=True)
        return Response({
            "message": "Approved marketplace products fetched successfully",
            "data": serializer.data
...
def getUnapprovedMarketplaceProducts(request):
    try:
        products = MarketPlaceProduct.objects.filter(isApproved=False)
        serializer = MarketPlaceProductSerializer(products, many=True)
        return Response({
            "message": "Unapproved marketplace products fetched successfully",
            "data": serializer.data
        }, status=status.HTTP_200_OK)
    except Exception as e:
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
@api_view(['PUT'])
def approveMarketplaceProduct(request, pk):
    try:
        product = MarketPlaceProduct.objects.get(pk=pk)
        product.isApproved = True
        product.save()

        # Get the user who submitted the product
        user = product.user

        # Compose the email
        subject = "Your Marketplace Product Has Been Approved ✅"
        message = f"""
Hi {user.first_name or user.username},

Great news! 🎉

Your product "{product.name}" has been reviewed and approved by our admin team. It is now live on the marketplace.
...
"""
        ...
        return Response({"message": "Marketplace Product approved and user notified via email."}, status=status.HTTP_200_OK)

    except MarketPlaceProduct.DoesNotExist:
        return Response({"error": "Marketplace Product not found"}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        print("Marketplace Product approval exception:")
        traceback.print_exc()
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
